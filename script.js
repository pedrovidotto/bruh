...
function renderWorkout(idx) {
  ...
  items.forEach((ex, i) => {
    ...
    li.addEventListener('pointerdown', (e) => {
      ...
      pressTimer = setTimeout(() => {
        ...
        // Stop timer if we're now below sTotal
    ...
    li.addEventListener('pointerup', (e) => {
      ...
      progress[id] = newVal;
      lastTouched[id] = Date.now();
      // Only start rest timer if we haven't just completed the last set
  ...
  // Sort active: most-recently-touched first
  ...
  // Day completion — only show once per session, re-guard after reset
...
function startTimer(sec) {
  ...
  tick();
  activeTimer = setInterval(tick, 500); // 500ms for responsiveness
}
...
function toggleGround(el) { el.classList.toggle('done'); }
...
function openBreatheModal(modeKey) {
  ...
  // Hard reset without transitions
  ...
  // Delay start to let reset paint settle
...
function init() {
  ...
  // Theme
  const saved = localStorage.getItem('workoutSysTheme');
